...
def startperfmon():
    while True:
        ...
        query="UPDATE perfmon set `date`=%s,`pc1`=TRUNCATE(%s,2),`pc2`=TRUNCATE(%s,2),`stat`= 'Online' where id= 2"
        mycursor=mydb.cursor()
        mycursor.execute(query,lr)
        mydb.commit()
        cmd()
        if not is_on:
            query="UPDATE perfmon set `pc1`=0, `pc2`=0, `stat`= 'Offline' where id = 2"
            mycursor=mydb.cursor()
...
def cmd():
    query="SELECT cmd FROM perfmon WHERE 2"
    mycursor=mydb.cursor()
    mycursor.execute(query)
